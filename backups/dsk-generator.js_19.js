/**
 * DSK Generator Engine para Amstrad CPC - VERSIÓN FINAL
 * (Geometría FDC + AMSDOS Header + Multi-Extent + Dir a 0x00)
 */
...
function createAmsdosHeader(filename, extension, type, length, loadAddr, execAddr) {
    ...
    header[18] = type; // 2 = Binario
    
    // Tamaños y direcciones (Little Endian)
...
function writeDirEntry(buffer, offset, filename, ext, extent, records, blocks) {
    // Inicializar a 0x00 para evitar "File Not Found"
    for (let i = 0; i < 32; i++) buffer[offset + i] = 0x00; 
    
    buffer[offset + 0] = 0; // User 0
...
function generateCPCDisk(screenData, mode, animate, fw, fh, nf) {
    const TRACKS = 40;
    const SECTORS = 9;
    const TRACK_DATA_SIZE = 4864; // 256 cabecera + 9*512
    
    const buffer = new Uint8Array(256 + (TRACKS * TRACK_DATA_SIZE));
    buffer.fill(0xE5); // Base vacía física del disco
    ...
    // --- 2. GENERACIÓN DE PISTAS FÍSICAS ---
    for (let t = 0; t < TRACKS; t++) {
        ...
        for (let s = 0; s < SECTORS; s++) {
            ...
            // Longitud del sector y estados de la FDC
            buffer[sOff + 4] = 0x00; 
            buffer[sOff + 5] = 0x00; 
            buffer[sOff + 6] = 0x00; 
            buffer[sOff + 7] = 0x02; 
    ...
    if (!animate) {
        ...
        loaderBasicStr = 
            ...
            `40 REM Inyectar rutina Ensamblador LDIR en &A000\r\n` +
            `50 FOR i=0 TO 20: READ a: POKE &A000+i, a: NEXT i\r\n` +
            `60 DATA 221,110,4,221,102,5,221,94,2,221,86,3,221,78,0,221,70,1,237,176,201\r\n` +
            `70 f=0: w=${wBytes}: h=${fh}: px=0\r\n` +
            `80 WHILE 1\r\n` +
            `90 FOR y=0 TO h-1\r\n` +
            `100 o = INT(y/8)*80 + (y MOD 8)*2048\r\n` +
            `110 s = &4000 + (f * w * h) + (y * w)\r\n` +
            `120 d = &C000 + o + px\r\n` +
            `130 CALL &A000, s, d, w\r\n` +
            `140 NEXT y\r\n` +
            `150 REM Sincronismo vertical\r\n` +
            `160 CALL &BD19\r\n` +
            `170 CALL &BD19\r\n` +
            `180 f = (f + 1) MOD ${nf}\r\n` +
            `190 WEND\r\n\x1A`;
    ...
    let remainingRecords = Math.ceil((fileBin.length - 16384) / 128);
    writeDirEntry(buffer, dirOffset + 64, "SCREEN  ", "BIN", 1, remainingRecords, [19]);

    // --- 5. INYECCIÓN A BLOQUES FÍSICOS ---
