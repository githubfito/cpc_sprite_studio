 ...
 * Genera el cargador BASIC con la línea 11 de INKs agrupados.
 */
...
function generateCPCDisk(screenData, mode, animate, fw, fh, nf, animSpeed) {
    ...
    if (!animate) {
        ...
        loader = `10 MEMORY &3FFF\r\n${inkLine}\r\n20 MODE ${mode}\r\n30 LOAD "SCREEN.BIN",&4000\r\n` +
                 ...
                 `60 px=0: py=100: dx=1\r\n70 CLS\r\n80 WHILE 1\r\n` +
                 `85 FOR i=1 TO ${delay}: CALL &BD19: NEXT i\r\n` + 
    ...
    const blocks = [];
    for(let i=0; i < Math.min(blocksCount, 16); i++) blocks.push(3 + i);
    writeDirEntry(buffer, dirOffset + 32, "SCREEN  ", "BIN", 0, Math.min(blocksCount * 8, 128), blocks);
