...
function generateCPCDisk(screenData, mode, animate, fw, fh, nf) {
    ...
    if (!animate) {
        // Modo estático normal con MEMORY para proteger memoria
        loaderBasicStr = `10 MEMORY &3FFF\r\n20 MODE ${mode}\r\n30 LOAD "SCREEN.BIN",&C000\r\n40 CALL &BB18\r\n50 GOTO 40\r\n\x1A`;
    } else {
        // Modo animado desde &4000 con MEMORY restrictivo y px anclado
        ...
        loaderBasicStr = 
            ...
            `40 f=0: w=${wBytes}: h=${fh}: px=0\r\n` +
            `50 WHILE 1\r\n` +
            `60 FOR y=0 TO h-1\r\n` +
            `70 o = INT(y/8)*80 + (y MOD 8)*2048\r\n` +
            `80 s = &4000 + (f * w * h) + (y * w)\r\n` +
            `90 d = &C000 + o + px\r\n` +
            `100 FOR bx=0 TO w-1\r\n` +
            `110 POKE d+bx, PEEK(s+bx)\r\n` +
            `120 NEXT bx\r\n` +
            `130 NEXT y\r\n` +
            `140 REM Sincronismo vertical\r\n` +
            `150 CALL &BD19\r\n` +
            `160 CALL &BD19\r\n` +
            `170 f = (f + 1) MOD ${nf}\r\n` +
            `180 WEND\r\n\x1A`;
