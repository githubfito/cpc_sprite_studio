/**
 * DSK Generator Engine para Amstrad CPC - FIX V-SYNC + MOVIMIENTO SUAVE
 */
...
function generateCPCDisk(screenData, mode, animate, fw, fh, nf) {
    ...
    if (!animate) {
        ...
        loaderBasicStr = 
            ...
            "60 px=0: py=100: dx=1\r\n" + 
            "70 CLS: PRINT \"W:\";w;\" H:\";h;\" NF:\";nf\r\n" +
            "80 WHILE 1\r\n" +
            "85 CALL &BD19\r\n" + // Wait Retrace antes de empezar a tocar memoria
    ...
    // Añadimos espacio extra para el buffer de borrado negro en el BIN
    const headerBin = createAmsdosHeader("SCREEN  ", "BIN", 2, screenData.length + (wBytes * fh) + 1, animate ? 0x4000 : 0xC000, 0x0000);
    const fileBin = new Uint8Array(128 + screenData.length + (wBytes * fh) + 1);
